...
/**
 * ============================================================================
 * PAGE : Score SEO Global
 * ============================================================================
 */

$scoreExists = false;
try {
    $stmt = $pdo->query("SELECT 1 FROM information_schema.tables WHERE table_name = 'seo_score' LIMIT 1");
    $scoreExists = $stmt->rowCount() > 0;
} catch (Exception $e) {}

if (!$scoreExists) {
    ?>
    <h1 class="page-title">Score SEO</h1>
    <div style="padding: 3rem; text-align: center; max-width: 600px; margin: 2rem auto;">
        <h2 style="margin-bottom: 1rem; color: var(--text-primary);">Score non calculé</h2>
        <p style="color: var(--text-secondary);">Lancez le calcul du score SEO.</p>
        <code style="display: block; background: var(--bg-secondary); padding: 1rem; border-radius: 8px; color: var(--text-secondary); font-size: 0.85rem; margin-top: 1rem;">
            php scouter-enricher/score.php <?= $crawlId ?>
        </code>
    </div>
    <?php
    return;
...
if (!$score) {
    ...
    <h1 class="page-title">Score SEO</h1>
    <div style="padding: 3rem; text-align: center; max-width: 600px; margin: 2rem auto;">
        <h2 style="color: var(--text-primary);">Pas de score pour ce crawl</h2>
        <code style="display: block; background: var(--bg-secondary); padding: 1rem; border-radius: 8px; color: var(--text-secondary); font-size: 0.85rem; margin-top: 1rem;">
            php scouter-enricher/score.php <?= $crawlId ?>
        </code>
    </div>
...
function scoreBarHtml($val, $max, $label) {
...
<div style="display: flex; flex-direction: column; gap: 1.5rem;">
    ...
    <div style="display: flex; gap: 2rem; align-items: center; flex-wrap: wrap;">
        ...
        <div style="flex: 1; min-width: 300px;">
            <?= scoreBarHtml($score->score_technique, 25, 'Technique') ?>
            <?= scoreBarHtml($score->score_contenu, 25, 'Contenu') ?>
            <?= scoreBarHtml($score->score_maillage, 25, 'Maillage interne') ?>
            <?= scoreBarHtml($score->score_enrichment, 15, 'Enrichissement') ?>
            <?php if ($score->score_performance !== null): ?>
                <?= scoreBarHtml($score->score_performance, 10, 'Performance') ?>
            <?php else: ?>
                <div style="margin-bottom: 0.75rem;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
    ...
    <div class="scorecards">
        <?php
        Component::card(['color' => getScoreColor($score->score_technique, 25), 'icon' => 'build', 'title' => 'Technique',
            'value' => $score->score_technique . '/25', 'desc' => 'HTTP, indexabilité, temps de réponse']);
        Component::card(['color' => getScoreColor($score->score_contenu, 25), 'icon' => 'article', 'title' => 'Contenu',
            'value' => $score->score_contenu . '/25', 'desc' => 'Balises SEO, richesse, duplication']);
        Component::card(['color' => getScoreColor($score->score_maillage, 25), 'icon' => 'hub', 'title' => 'Maillage',
            'value' => $score->score_maillage . '/25', 'desc' => 'Profondeur, inlinks, redirections']);
        Component::card(['color' => getScoreColor($score->score_enrichment, 15), 'icon' => 'auto_awesome', 'title' => 'Enrichissement',
    ...
    <div class="charts-grid">
        <?php
        // Radar chart via donut (représentation des 4-5 axes)
        $radarData = [
        ...
        Component::chart([
            'type' => 'donut', 'title' => 'Répartition du score',
        ...
        <div class="chart-card" style="overflow-y: auto;">
            ...
            <div style="padding: 0 1rem 1rem;">
                <?php
                $criteriaLabels = [
                    'pages_200_rate' => 'Pages en 200',
                    'indexability_rate' => 'Taux d\'indexabilité',
                    'error_pages' => 'Pages en erreur',
                    'avg_response_time' => 'Temps de réponse moyen',
                    'title_unique_rate' => 'Titles uniques',
                    'h1_unique_rate' => 'H1 uniques',
                    'metadesc_rate' => 'Meta descriptions',
                    'rich_content_rate' => 'Contenu riche (300+ mots)',
                    'duplication_rate' => 'Taux de duplication',
                    'avg_depth' => 'Profondeur moyenne',
                    'orphan_rate' => 'Pages orphelines',
                    'bad_redirect_chains' => 'Redirections en erreur',
                    'pr_concentration' => 'Concentration PageRank',
                    'images_alt_rate' => 'Images avec alt',
                    'opengraph_rate' => 'Open Graph',
                    'schema_rate' => 'Données structurées',
                ...
                $sections = [
                    ...
                    'Maillage' => $details['maillage'] ?? [],
                    'Enrichissement' => $details['enrichment'] ?? [],
                ...
                ?>
                    <div style="margin-bottom: 1rem;">
                        <div style="font-weight: 600; font-size: 0.85rem; color: var(--text-secondary); text-transform: uppercase; margin-bottom: 0.5rem; letter-spacing: 0.5px;">
                            <?= $sectionName ?>
                        </div>
                        ...
                        ?>
                            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.4rem 0; border-bottom: 1px solid var(--border-color);">
                                ...
                                <div style="display: flex; align-items: center; gap: 0.75rem;">
                                    ...
                                    <span style="font-size: 0.8rem; font-weight: 600; color: <?= $item['points'] >= $item['max'] * 0.8 ? '#34a853' : ($item['points'] >= $item['max'] * 0.5 ? '#fbbc05' : '#ea4335') ?>;">
