...
/**
 * ============================================================================
 * PAGE : Analyse Open Graph & Twitter Cards
 * ============================================================================
 */

$enrichmentExists = false;
try {
    $stmt = $pdo->query("SELECT 1 FROM information_schema.tables WHERE table_name = 'page_opengraph' LIMIT 1");
    $enrichmentExists = $stmt->rowCount() > 0;
} catch (Exception $e) {}

if (!$enrichmentExists || !$crawlId) {
    ?>
    ...
    <div style="padding: 3rem; text-align: center; max-width: 600px; margin: 2rem auto;">
        ...
        <p style="color: var(--text-secondary);">Lancez le script d'enrichissement.</p>
...
if ($stmt->fetchColumn() == 0) {
    ...
    <h1 class="page-title">Open Graph</h1>
    <div style="padding: 3rem; text-align: center; max-width: 600px; margin: 2rem auto;">
        <h2 style="color: var(--text-primary);">Pas de données pour ce crawl</h2>
        <code style="display: block; background: var(--bg-secondary); padding: 1rem; border-radius: 8px; color: var(--text-secondary); font-size: 0.85rem; margin-top: 1rem;">
            php scouter-enricher/enrich.php <?= $crawlId ?>
        </code>
    </div>
...
$stmt = $pdo->prepare("
    ...
    WHERE og.crawl_id = :cid AND p.compliant = true
        ...
        AND (og.og_image IS NULL OR og.og_image = '')
...
// Pages avec OG image mais pas de og:title
$stmt = $pdo->prepare("
    SELECT p.url, og.og_image
    FROM page_opengraph og
    JOIN pages p ON p.crawl_id = og.crawl_id AND p.id = og.page_id
    WHERE og.crawl_id = :cid
        AND (og.og_title IS NULL OR og.og_title = '')
        AND og.og_image IS NOT NULL AND og.og_image != ''
    LIMIT 30
");
$stmt->execute([':cid' => $crawlId]);
$ogImageNoTitle = $stmt->fetchAll();
...
<div style="display: flex; flex-direction: column; gap: 1.5rem;">
    ...
    <div class="scorecards">
        ...
        Component::card(['color' => 'success', 'icon' => 'share', 'title' => 'og:title',
            'value' => number_format($kpis->with_og_title), 'desc' => "{$ogPct}% des pages indexables"]);
        Component::card(['color' => 'info', 'icon' => 'image', 'title' => 'og:image',
            'value' => number_format($kpis->with_og_image), 'desc' => "{$imgPct}% des pages indexables"]);
    ...
    <div class="charts-grid">
        ...
        Component::chart([
            'type' => 'donut', 'title' => 'Couverture Open Graph',
        ...
        if (!empty($typeDist)) {
            ...
            foreach ($typeDist as $t) {
                $typeDonut[] = ['name' => $t->og_type, 'y' => (int)$t->count];
            }
            Component::chart([
                'type' => 'donut', 'title' => 'Distribution og:type',
    ...
    <?php if (!empty($pagesNoOg)):
        $noOgTable = [];
        foreach ($pagesNoOg as $row) {
            $noOgTable[] = ['page' => parse_url($row->url, PHP_URL_PATH) ?: $row->url];
        }
        Component::simpleTable([
            'title' => 'Pages sans Open Graph (' . count($pagesNoOg) . ')',
            'subtitle' => 'Pages indexables sans og:title ni og:image',
            'columns' => [['key' => 'page', 'label' => 'Page', 'type' => 'bold']],
            'data' => $noOgTable
        ]);
    endif; ?>
