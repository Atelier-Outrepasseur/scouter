...
$psExists = false;
try {
    $stmt = $pdo->query("SELECT 1 FROM information_schema.tables WHERE table_name = 'page_pagespeed' LIMIT 1");
    $psExists = $stmt->rowCount() > 0;
} catch (Exception $e) {}
...
if (!$psExists) {
    ...
    <div style="padding: 3rem; text-align: center; max-width: 600px; margin: 2rem auto;">
        <h2 style="margin-bottom: 1rem; color: var(--text-primary);">PageSpeed non configuré</h2>
        <p style="color: var(--text-secondary);">Lancez l'analyse PageSpeed via scouter-enricher.</p>
        <code style="display: block; background: var(--bg-secondary); padding: 1rem; border-radius: 8px; color: var(--text-secondary); font-size: 0.85rem; margin-top: 1rem;">
            php scouter-enricher/pagespeed.php <?= $crawlId ?>
        </code>
    </div>
    <?php
    return;
...
if ($stmt->fetchColumn() == 0) {
    ...
    <h1 class="page-title">PageSpeed Insights</h1>
    <div style="padding: 3rem; text-align: center; max-width: 600px; margin: 2rem auto;">
        <h2 style="margin-bottom: 1rem; color: var(--text-primary);">Pas de données PageSpeed</h2>
        <p style="color: var(--text-secondary);">Lancez l'analyse :</p>
        <code style="display: block; background: var(--bg-secondary); padding: 1rem; border-radius: 8px; color: var(--text-secondary); font-size: 0.85rem; margin-top: 1rem;">
            php scouter-enricher/pagespeed.php <?= $crawlId ?>
        </code>
    </div>
...
// Distribution des scores performance
$stmt = $pdo->prepare("
    SELECT 
        CASE 
...
$stmt = $pdo->prepare("
    SELECT 
        ...
        END as range,
        COUNT(*) as count
    FROM page_pagespeed WHERE crawl_id = :cid AND lcp IS NOT NULL
    GROUP BY range
");
$stmt->execute([':cid' => $crawlId]);
...
$stmt = $pdo->prepare("
    ...
    FROM page_pagespeed WHERE crawl_id = :cid
    ORDER BY score_performance ASC
");
$stmt->execute([':cid' => $crawlId]);
$allPages = $stmt->fetchAll();

// Top opportunités (agrégées sur toutes les pages)
$stmt = $pdo->prepare("
    SELECT opportunities FROM page_pagespeed WHERE crawl_id = :cid AND opportunities != '[]'
");
$stmt->execute([':cid' => $crawlId]);
...
function metricStatus($metric, $good, $poor) {
    if ($metric <= $good) return '🟢';
    if ($metric <= $poor) return '🟠';
    return '🔴';
}
...
<div style="display: flex; flex-direction: column; gap: 1.5rem;">
    ...
    <div class="charts-grid">
        ...
        $perfDonut = [];
        foreach ($perfDist as $p) {
            $perfDonut[] = ['name' => $p->range, 'y' => (int)$p->count, 'color' => $perfColors[$p->range] ?? '#666'];
        }
        Component::chart([
            'type' => 'donut', 'title' => 'Distribution scores performance',
            'subtitle' => 'Répartition des pages par niveau',
        ...
        $lcpDonut = [];
        foreach ($lcpDist as $l) {
            $lcpDonut[] = ['name' => $l->range, 'y' => (int)$l->count, 'color' => $lcpColors[$l->range] ?? '#666'];
        }
        Component::chart([
            'type' => 'donut', 'title' => 'Distribution LCP',
    ...
    foreach ($allPages as $p) {
        $pageTable[] = [
            'url' => parse_url($p->url, PHP_URL_PATH) ?: $p->url,
            'perf' => $p->score_performance . '/100',
            'fcp' => $p->fcp . 's',
            'lcp' => $p->lcp . 's',
            'tbt' => round($p->tbt) . 'ms',
            'cls' => $p->cls,
    ...
    Component::simpleTable([
        'title' => 'Détail par page',
        'subtitle' => 'Triées du score le plus faible au plus élevé',
        'columns' => [
            ['key' => 'url', 'label' => 'Page', 'type' => 'bold'],
            ['key' => 'perf', 'label' => 'Perf.', 'type' => 'default'],
            ['key' => 'fcp', 'label' => 'FCP', 'type' => 'default'],
            ['key' => 'lcp', 'label' => 'LCP', 'type' => 'default'],
            ['key' => 'tbt', 'label' => 'TBT', 'type' => 'default'],
            ['key' => 'cls', 'label' => 'CLS', 'type' => 'default'],
            ['key' => 'si', 'label' => 'Speed Index', 'type' => 'default'],
        ],
        'data' => $pageTable
    ]);
    ?>
    ...
    <?php if (!empty($oppAggregated)):
        $oppTable = [];
        foreach ($oppAggregated as $opp) {
            $oppTable[] = [
                'title' => $opp['title'],
                'savings' => round($opp['total_savings'] / 1000, 1) . 's',
                'pages' => $opp['pages'] . ' pages',
            ];
        }
        Component::simpleTable([
            'title' => 'Opportunités d\'amélioration',
            'subtitle' => 'Recommandations agrégées sur toutes les pages analysées',
            'columns' => [
                ['key' => 'title', 'label' => 'Recommandation', 'type' => 'bold'],
                ['key' => 'savings', 'label' => 'Gain potentiel', 'type' => 'default'],
                ['key' => 'pages', 'label' => 'Pages concernées', 'type' => 'default'],
            ],
            'data' => $oppTable
        ]);
    endif; ?>
